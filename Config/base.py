...
class BaseAuthInterface(ABC):
    ...
    def create_user_response_with_tokens(
        self, user_record: UserRecord, provider: str = None, provider_uid: str = None
    ) -> Dict[str, Any]:
        """
    ...
    def create_success_response(
        ...
        """
        response_data = {"success": True, "data": data, "timestamp": time.time()}
    ...
    def create_error_response(message: str, status_code: int = 400) -> Response:
        ...
        Returns Flask Response object
        """
        response_data = {"success": False, "error": message, "timestamp": time.time()}
        response = jsonify(response_data)
        response.status_code = status_code
        response.headers["Access-Control-Allow-Origin"] = "*"
        return response
